...
describe 'cloud::clustering::pacemaker_service', :type => :define do  
  ...
  context 'with colocated services and start ordering' do
    before :each do
      params.merge!(
        :colocated_services => ["foo-api,bar-api"],
        :start_order        => ["bar-api,foo-api"],
      )
    end
      
    it 'creates a colocation constraint' do
      is_expected.to contain_cloud__clustering__pacemaker_colocation('foo-api,bar-api')
    end
  
    it 'creates an order constraint' do
      is_expected.to contain_cloud__clustering__pacemaker_order('bar-api,foo-api')
    end
  end
