...
class cloud::monitoring::server::sensu (
  ...
  ensure_resource('@@rabbitmq_user', $rabbitmq_user, { password => $rabbitmq_password })
  ensure_resource('@@rabbitmq_vhost', $rabbitmq_vhost, {  ensure  => present })
  ensure_resource('@@rabbitmq_user_permissions', "${rabbitmq_user}@${rabbitmq_vhost}", {
