...
class cloud::monitoring::server::sensu (
  ...
  $rabbitmq_user_exported = query_nodes("@@Rabbitmq_user['${rabbitmq_user}']")
  $rabbitmq_vhost_exported = query_nodes("@@Rabbitmq_vhost['${rabbitmq_vhost}']")
  $rabbitmq_user_permissions_exported = query_nodes("@@Rabbitmq_user_permissions['${rabbitmq_user}@${rabbitmq_vhost}']")

  if size($rabbitmq_user_exported) == 0 {
    @@rabbitmq_user { $rabbitmq_user :
      password => $rabbitmq_password,
    }
  }
  if size($rabbitmq_vhost_exported) == 0 {
    @@rabbitmq_vhost { $rabbitmq_vhost :
      ensure  => present,
    }
  }
  if size($rabbitmq_user_permissions_exported) == 0 {
